...
public class TicTacToeBoard {
    ...
    public boolean move(String input, char mark) throws InvalidInputException,
    	...
    	if (input == null) throw new NullInputException(); //#1
    	
    	if (input.length() != 2) throw new InvalidInputException(); //#2
    	
    	input = input.toUpperCase();
    	char rowChar = input.charAt(0);
    	char colChar = input.charAt(1);
    	if (rowChar < 'A' || rowChar > 'C' || colChar < '1' || colChar > '3') throw new InvalidInputException();
    ...
    public boolean move(int row, int col, char mark) 
    	...
    	if (grid[row][col] != SPACE) throw new IllegalMoveException(); //#1
    	
    	grid[row][col] = mark; //#2
    	
    	return isWin(mark) || isTie();
    ...
    boolean isWin(char mark) {
        ...
        //Checking for diagonal win (Upper Left - Lower Right)
        boolean win = true;
        for (int x = 0; x < 3; x++) {
            if (grid[x][x] != mark) win = false;
        }
        if (win) {
            gameHasBeenWon = true;
            return gameHasBeenWon;
        ...
        //Checking for diagonal win (Lower Left - Upper Right)
        win = true;
        for (int x = 0; x < 3; x++) {
            if (grid[x][2 - x] != mark) win = false;
        }
        if (win) {
            gameHasBeenWon = true;
            return gameHasBeenWon;
